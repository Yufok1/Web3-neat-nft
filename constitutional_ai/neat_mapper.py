...
class TraitToNEATMapper:
    ...
    def map_population_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processing_speed = self.normalize_trait("ProcessingSpeed", traits.get("ProcessingSpeed", 1.0))
        energy_efficiency = self.normalize_trait("EnergyEfficiency", traits.get("EnergyEfficiency", 1.0))
        social_drive = self.normalize_trait("SocialDrive", traits.get("SocialDrive", 1.5))
        ...
        population_size = int(self._map_to_range(pop_factor, self.base_ranges['population_size']))
        
        # Elitism: more stable agents preserve more elite individuals
        elitism_factor = stability * 0.8 + energy_efficiency * 0.2
        elitism_count = int(self._map_to_range(elitism_factor, self.base_ranges['elitism_count']))
        
        # Species count: social agents maintain more diverse species
        species_factor = social_drive * 0.7 + stability * 0.3
        species_count = int(self._map_to_range(species_factor, self.base_ranges['species_count_target']))
    ...
    def map_architecture_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        working_memory = self.normalize_trait("WorkingMemory", traits.get("WorkingMemory", "Low"))
        expertise = self.normalize_trait("Expertise", traits.get("Expertise", 1.0))
        attention_span = self.normalize_trait("AttentionSpan", traits.get("AttentionSpan", 3.0))
        
        # Initial hidden nodes: based on perception and working memory
        hidden_factor = perception * 0.6 + working_memory * 0.4
        initial_hidden = int(self._map_to_range(hidden_factor, self.base_ranges['initial_hidden_nodes']))
        ...
        max_nodes = int(self._map_to_range(max_nodes_factor, self.base_ranges['max_nodes']))
        
        # Max connections: based on perception and expertise
        max_conn_factor = perception * 0.4 + expertise * 0.4 + working_memory * 0.2
        max_connections = int(self._map_to_range(max_conn_factor, self.base_ranges['max_connections']))
    ...
    def map_mutation_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Normalize relevant traits
        innovation_drive = self.normalize_trait("InnovationDrive", traits.get("InnovationDrive", 1.0))
        risk_tolerance = self.normalize_trait("RiskTolerance", traits.get("RiskTolerance", 1.5))
        stability = self.normalize_trait("Stability", traits.get("Stability", 1.5))
        ...
        weight_mutation_rate = self._map_to_range(weight_mut_factor, self.base_ranges['weight_mutation_rate'])
        ...
        weight_perturbation_rate = self._map_to_range(perturb_factor, self.base_ranges['weight_perturbation_rate'])
        
        # Add node rate: driven by curiosity and innovation
        add_node_factor = curiosity * 0.5 + innovation_drive * 0.5
        add_node_rate = self._map_to_range(add_node_factor, self.base_ranges['add_node_rate'])
        
        # Add connection rate: similar to add node but slightly more conservative
        add_conn_factor = innovation_drive * 0.6 + curiosity * 0.4
        add_connection_rate = self._map_to_range(add_conn_factor, self.base_ranges['add_connection_rate'])
        
        # Remove connection rate: inversely related to stability
        remove_conn_factor = (1.0 - stability) * 0.6 + risk_tolerance * 0.4
        remove_connection_rate = self._map_to_range(remove_conn_factor, self.base_ranges['remove_connection_rate'])
    ...
    def map_learning_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Normalize relevant traits
        learning_rate_trait = self.normalize_trait("LearningRate", traits.get("LearningRate", 0.1))
        transfer_learning = self.normalize_trait("TransferLearning", traits.get("TransferLearning", 1.0))
        stability = self.normalize_trait("Stability", traits.get("Stability", 1.5))
        
        # Learning rate: directly from LearningRate trait
        learning_rate = self._map_to_range(learning_rate_trait, self.base_ranges['learning_rate'])
        ...
        weight_decay = self._map_to_range(decay_factor, self.base_ranges['weight_decay'])
    ...
    def map_selection_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compatibility_threshold = self._map_to_range(compat_factor, self.base_ranges['compatibility_threshold'])
        
        # Survival rate: expert, stable agents can afford to be more selective
        survival_factor = 1.0 - (expertise * 0.6 + stability * 0.4)
        survival_rate = self._map_to_range(survival_factor, self.base_ranges['survival_rate'])
    ...
    def map_performance_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        energy_efficiency = self.normalize_trait("EnergyEfficiency", traits.get("EnergyEfficiency", 1.0))
        processing_speed = self.normalize_trait("ProcessingSpeed", traits.get("ProcessingSpeed", 1.0))
        meta_learning = self.normalize_trait("MetaLearning", traits.get("MetaLearning", 0.5))
        
        # Max generations: based on attention span and efficiency
        max_gen_factor = attention_span * 0.6 + energy_efficiency * 0.4
        max_generations = int(self._map_to_range(max_gen_factor, self.base_ranges['max_generations']))
        ...
        fitness_threshold = self._map_to_range(fitness_factor, self.base_ranges['fitness_threshold'])
        
        # Stagnation threshold: patient, efficient agents wait longer
        stagnation_factor = attention_span * 0.5 + energy_efficiency * 0.5
        stagnation_threshold = int(self._map_to_range(stagnation_factor, self.base_ranges['stagnation_threshold']))
    ...
    def map_behavioral_parameters(self, traits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        exploration_rate = self._map_to_range(exploration_factor, self.base_ranges['exploration_rate'])
        
        # Temperature for softmax decisions: innovation drives randomness
        temperature_factor = innovation_drive * 0.7 + risk_tolerance * 0.3
        temperature = self._map_to_range(temperature_factor, self.base_ranges['temperature'])
    ...
    def _map_to_range(self, normalized_value: float, value_range: Tuple[float, float]) -> float:
        """
    ...
    def validate_monotonicity(self, traits1: Dict[str, Any], traits2: Dict[str, Any]) -> bool:
        """
