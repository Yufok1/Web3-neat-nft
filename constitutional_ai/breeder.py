...
from .traits import get_trait_definitions
...
class ConstitutionalBreeder:
    ...
    def _crossover_breeding(
            ...
            if self._should_respect_linkage(group_id):
                ...
                for trait_name in trait_names:
                    if trait_name in donor_parent.loci:
                        offspring.loci[trait_name] = copy.deepcopy(
                            donor_parent.loci[trait_name]
                        )
            else:
                ...
                for trait_name in trait_names:
                    if trait_name in common_traits:
                        new_locus = self._crossover_locus(
                            parent1.loci[trait_name], parent2.loci[trait_name]
                        )
        ...
        offspring.provenance = ProvenanceRecord(
            ...
            generation=max(parent1.provenance.generation, parent2.provenance.generation)
            + 1,
    ...
    def _clone_with_mutation(
        self, parent: ConstitutionalGenome, seed: Optional[int]
    ) -> BreedingResult:
        """
    ...
    def _organize_by_linkage(
        self, parent1: ConstitutionalGenome, parent2: ConstitutionalGenome
    ) -> Dict[int, List[str]]:
        """
    ...
    def _crossover_locus(self, locus1: Locus, locus2: Locus) -> Locus:
        ...
        maternal_allele = random.choice(
            [maternal_source.maternal_allele, maternal_source.paternal_allele]
        )
        paternal_allele = random.choice(
            [paternal_source.maternal_allele, paternal_source.paternal_allele]
        )
    ...
    def _mutate_numeric_allele(self, allele: Allele, trait_def: dict):
        ...
        delta = random.uniform(
            -self.config.max_mutation_delta, self.config.max_mutation_delta
        )
    ...
    def create_random_population(
        self, size: int, trait_set: str = "complete", seed: Optional[int] = None
    ) -> List[ConstitutionalGenome]:
        """
    ...
    def _create_random_genome(
        self, trait_defs: Dict[str, dict], seed: Optional[int]
    ...
    def create_specialized_starters(
        self, seed: Optional[int] = None
    ) -> Dict[str, IdentityBundle]:
        """
        ...
        from .genome import ConstitutionalGenome, Locus, Allele
            ...
            for locus_name, locus_data in genome_def["loci"].items():
                maternal_allele = Allele(
                    ...
                    allele_type=AlleleType(
                        locus_data["maternal_allele"]["allele_type"]
                    ),
                ...
                paternal_allele = Allele(
                    ...
                    allele_type=AlleleType(
                        locus_data["paternal_allele"]["allele_type"]
                    ),
    ...
    def get_breeding_statistics(self, results: List[BreedingResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "crossover_rate": (
                crossover_count / total_results if total_results > 0 else 0
            ),
            "clone_rate": clone_count / total_results if total_results > 0 else 0,
            "avg_mutations_per_offspring": (
                total_mutations / total_results if total_results > 0 else 0
            ),
            "avg_crossovers_per_offspring": (
                total_crossovers / total_results if total_results > 0 else 0
            ),
            "avg_linkage_breaks_per_offspring": (
                total_linkage_breaks / total_results if total_results > 0 else 0
            ),
...
def create_initial_population(
    size: int = 100, trait_set: str = "complete", seed: Optional[int] = None
) -> List[ConstitutionalGenome]:
    """
