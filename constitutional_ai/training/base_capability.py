...
class CapabilityFitnessEvaluator(ABC):
    ...
    def evaluate_network(
        self, network: neat.nn.FeedForwardNetwork, num_tests: int = 100
    ) -> float:
        """
    ...
    def evaluate_generation_quality(
        self, network: neat.nn.FeedForwardNetwork, **kwargs
    ) -> EvaluationResult:
        """
...
class CapabilityTrainingPipeline(ABC):
    ...
    def train_capability(self, generations: int = 50, **kwargs) -> Dict[str, Any]:
        ...
        """
        print(
            f"Training {self.capability_type} capability for agent {self.agent_identity.id_hash[:12]}..."
        )
        ...
        for trait_name in relevant_traits:
            trait_value = self.agent_identity.constitution_result.constitution.get(
                trait_name, "N/A"
            )
            print(f"  - {trait_name}: {trait_value}")

        # Create config file (runner handles trait-based configuration)
        config_file = self.neat_runner.create_neat_config_file(
            f"{self.capability_type}_config.txt"
        )
    ...
    def get_relevant_traits(self) -> List[str]:
        """Get list of constitutional traits relevant to this capability."""
        pass
...
def train_agent_capability(
    ...
    **kwargs,
    ...
    capability_modules = {
        "language": (
            "constitutional_ai.training.language_evolution",
            "LanguageTrainingPipeline",
        ),
        # Future capabilities can be added here:
        # 'coding': ('constitutional_ai.training.coding_evolution',
        #            'CodingTrainingPipeline'),
        # 'image': ('constitutional_ai.training.image_evolution',
        #           'ImageTrainingPipeline'),
        # 'reasoning': ('constitutional_ai.training.reasoning_evolution',
        #               'ReasoningTrainingPipeline'),
    }
    ...
    if training_data is None:
        if capability_type == "language":
            ...
            raise ValueError(
                f"No default training data available for {capability_type}"
            )
