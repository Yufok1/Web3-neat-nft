...
class Allele:
    ...
    domain: Optional[Tuple] = None  # (min, max) for numeric, tuple of choices for categorical
    
    def __post_init__(self):
        """Validate allele value against its type and domain."""
        if self.allele_type == AlleleType.NUMERIC:
            if not isinstance(self.value, (int, float)):
                raise ValueError(f"Numeric allele must have numeric value, got {type(self.value)}")
            if self.domain and not (self.domain[0] <= self.value <= self.domain[1]):
                raise ValueError(f"Numeric allele {self.value} outside domain {self.domain}")
                
        elif self.allele_type == AlleleType.CATEGORICAL:
            if self.domain and self.value not in self.domain:
                raise ValueError(f"Categorical allele {self.value} not in domain {self.domain}")
                
        elif self.allele_type == AlleleType.BOOLEAN:
            if not isinstance(self.value, bool):
                raise ValueError(f"Boolean allele must have boolean value, got {type(self.value)}")
...
class Locus:
    ...
    def get_dominant_allele(self) -> Allele:
        ...
        if (self.maternal_allele.allele_type == AlleleType.NUMERIC and 
            self.paternal_allele.allele_type == AlleleType.NUMERIC):
    ...
    def _resolve_stabilization_dominance(self, maternal_stab: StabilizationType, 
                                       paternal_stab: StabilizationType) -> StabilizationType:
        """
...
def create_random_genome(trait_definitions: Dict[str, Any], seed: Optional[int] = None) -> ConstitutionalGenome:
    """
    ...
    for trait_name, trait_spec in trait_definitions.items():
        ...
        if allele_type == AlleleType.NUMERIC:
            ...
            maternal_allele = Allele(random.choice([True, False]), allele_type, maternal_stab)
            paternal_allele = Allele(random.choice([True, False]), allele_type, paternal_stab)
