...
def traits_to_hue(traits: Dict[str, Any]) -> float:
    ...
    cognitive_strength = sum(normalized.get(t, 0) for t in cognitive_traits) / len(cognitive_traits)
    learning_strength = sum(normalized.get(t, 0) for t in learning_traits) / len(learning_traits)  
    behavioral_strength = sum(normalized.get(t, 0) for t in behavioral_traits) / len(behavioral_traits)
    performance_strength = sum(normalized.get(t, 0) for t in performance_traits) / len(performance_traits)
    social_strength = sum(normalized.get(t, 0) for t in social_traits) / len(social_traits)
    advanced_strength = sum(normalized.get(t, 0) for t in advanced_traits) / len(advanced_traits)
    
    # Map to hue regions (in degrees)\n    # Cognitive: 180-240 (Blue-Cyan)\n    # Learning: 120-180 (Green)\n    # Behavioral: 60-120 (Yellow-Orange)\n    # Performance: 300-360 + 0-60 (Red-Magenta)\n    # Social: 240-300 (Violet)\n    # Advanced: Modifies the blend\n    \n    hue_components = [\n        cognitive_strength * 210,      # Blue-cyan center\n        learning_strength * 150,       # Green center\n        behavioral_strength * 90,      # Yellow-orange center\n        performance_strength * 330,    # Red-magenta center (wraps around)\n        social_strength * 270,         # Violet center\n    ]\n    \n    # Weight by category strengths\n    weights = [cognitive_strength, learning_strength, behavioral_strength, \n               performance_strength, social_strength]\n    \n    if sum(weights) > 0:\n        # Weighted average of hue components\n        weighted_hue = sum(h * w for h, w in zip(hue_components, weights)) / sum(weights)\n        \n        # Advanced traits create \"shimmer\" - slight hue shifts\n        shimmer = advanced_strength * 30 * math.sin(sum(normalized.values()) * 2 * math.pi)\n        \n        final_hue = (weighted_hue + shimmer) % 360\n        return final_hue\n    \n    return 180.0  # Default blue if no traits
