...
class BlockchainManager:
    ...
    def create_genome_metadata(self, genome: Genome, generation: int) -> dict:
        ...
        metadata = {
            ...
            "image": f"https://via.placeholder.com/400x400/1a1a1a/ffffff?text=AI+Genome",  # Placeholder
            "attributes": [
                {
                ...
                {
                    "trait_type": "Species ID",
                    "value": genome.species_id
                },
                {
                    "trait_type": "Node Count",
                    "value": len(genome.nodes)
                },
                {
                    "trait_type": "Connection Count",
                    "value": len(genome.connections)
                },
                {
                    "trait_type": "Input Size",
                    "value": genome.input_size
                },
                {
                    "trait_type": "Output Size",
                    "value": genome.output_size
                }
            ],
    ...
    def mint_genome_nft(self, 
                       genome: Genome, 
                       generation: int,
                       recipient_address: str) -> Optional[Tuple[int, str]]:
        """
        ...
        try:
            ...
            if receipt.status == 1:
                ...
                token_id = len(self.get_owned_tokens(recipient_address))  # Approximation
    ...
    def get_genome_info(self, token_id: int) -> Optional[dict]:
        ...
        try:
            genome_hash, fitness, generation, species_id = self.contract.functions.getGenomeInfo(token_id).call()
